...
class Receita(db.Model):
    ...
    chef_id = db.Column(db.Integer, db.ForeignKey('chef.id'), nullable=False)
    ingredientes = db.relationship('ReceitaIngrediente', backref='receita', lazy=True)

class Ingrediente(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    nome = db.Column(db.String(100), unique=True, nullable=False)
    receitas = db.relationship('ReceitaIngrediente', backref='ingrediente', lazy=True)
    ...
    if request.method == 'POST':
        ...
        for nome in ingredientes_nomes:
            ...
            if not ingrediente:
                ...
                db.session.commit()
            receita_ingrediente = ReceitaIngrediente(receita_id=nova_receita.id, ingrediente_id=ingrediente.id, quantidade='1 unidade')
